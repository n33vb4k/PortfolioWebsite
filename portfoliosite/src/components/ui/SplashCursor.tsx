...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const copyProgram = new GLProgram(baseVert, copyShader);
    const clearProgram = new GLProgram(baseVert, clearShader);
    const splatProgram = new GLProgram(baseVert, splatShader);
    const advectionProgram = new GLProgram(baseVert, advectionShader);
    const divergenceProgram = new GLProgram(baseVert, divergenceShader);
    const curlProgram = new GLProgram(baseVert, curlShader);
    const vorticityProgram = new GLProgram(baseVert, vorticityShader);
    const pressureProgram = new GLProgram(baseVert, pressureShader);
    const gradienSubtractProgram = new GLProgram(baseVert, gradientSubtractShader);
    const displayMaterial = new Material(baseVert, displayShaderSource);
    ...
    function generateColor(): ColorRGB {
      // Constrain to indigo-violet hue range (235°–275° → 0.653–0.764)
      const h = 0.653 + Math.random() * 0.111, s = 0.8 + Math.random() * 0.2, v = 1;
      const i = Math.floor(h*6), f = h*6-i, p = v*(1-s), q = v*(1-f*s), t = v*(1-(1-f)*s);
      let r=0,g=0,b=0;
      switch(i%6){case 0:r=v;g=t;b=p;break;case 1:r=q;g=v;b=p;break;case 2:r=p;g=v;b=t;break;case 3:r=p;g=q;b=v;break;case 4:r=t;g=p;b=v;break;case 5:r=v;g=p;b=q;break;}
      return { r: r*0.025, g: g*0.025, b: b*0.025 };
    }
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      if (splatProgram.uniforms.radius) gl.uniform1f(splatProgram.uniforms.radius, r);
    ...
    function clickSplat(p: Pointer) { const c = generateColor(); c.r*=2;c.g*=2;c.b*=2; splat(p.texcoordX, p.texcoordY, 5*(Math.random()-0.5), 10*(Math.random()-0.5), c); }
    ...
    function render(target: any) {
      ...
      if (config.SHADING && displayMaterial.uniforms.texelSize) gl.uniform2f(displayMaterial.uniforms.texelSize, 1/(target?target.width:gl.drawingBufferWidth), 1/(target?target.height:gl.drawingBufferHeight));
      if (displayMaterial.uniforms.uTexture) gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0));
      blit(target, false);
    }

    function updateFrame() {
      const now = Date.now(); let dt = Math.min((now - lastUpdateTime) / 1000, 0.016666); lastUpdateTime = now;
      ...
      step(dt); render(null); requestAnimationFrame(updateFrame);
    }
    ...
    window.addEventListener('mousedown', e => {
      const p=pointers[0]; updatePointerDownData(p,-1,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY)); clickSplat(p);
    });
    function handleFirstMove(e: MouseEvent) {
      const p=pointers[0]; updateFrame(); updatePointerMoveData(p,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY),generateColor());
    ...
    window.addEventListener('mousemove', e => {
      const p=pointers[0]; updatePointerMoveData(p,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY),p.color);
    });
    function handleFirstTouch(e: TouchEvent) {
      const p=pointers[0];
      for(let i=0;i<e.targetTouches.length;i++){updateFrame();updatePointerDownData(p,e.targetTouches[i].identifier,scaleByPixelRatio(e.targetTouches[i].clientX),scaleByPixelRatio(e.targetTouches[i].clientY));}
      document.body.removeEventListener('touchstart', handleFirstTouch);
    }
    document.body.addEventListener('touchstart', handleFirstTouch);
    window.addEventListener('touchstart', e=>{const p=pointers[0];for(let i=0;i<e.targetTouches.length;i++)updatePointerDownData(p,e.targetTouches[i].identifier,scaleByPixelRatio(e.targetTouches[i].clientX),scaleByPixelRatio(e.targetTouches[i].clientY));},false);
    window.addEventListener('touchmove', e=>{const p=pointers[0];for(let i=0;i<e.targetTouches.length;i++)updatePointerMoveData(p,scaleByPixelRatio(e.targetTouches[i].clientX),scaleByPixelRatio(e.targetTouches[i].clientY),p.color);},false);
